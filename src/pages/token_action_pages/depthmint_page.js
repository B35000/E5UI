...
class DepthMintPage extends Component {
    ...
    state = {
        selected: 0,id:makeid(8), type: this.props.app_state.loc['898']/* 'depthmint' */, entered_indexing_tags:[this.props.app_state.loc['899']/* 'depth' */, this.props.app_state.loc['882']/* 'mint' */, this.props.app_state.loc['883']/* 'token' */], token_item: {'balance':1, 'data':[[],[],[],[],[]], 'id':0}, new_authmint_action_page_tags_object: this.get_new_depthmint_action_page_tags_object(),
    ...
    get_new_depthmint_action_page_tags_object(){
        return{
            ...
            'e':[
                ['xor','',0], ['e',this.props.app_state.loc['898']/* 'depthmint' */], [1]
            ],
        };
    }
    ...
    render(){
        return(
            <div style={{'padding':'10px 10px 0px 10px'}}>
                ...
                {this.render_detail_item('4', {'font':'Sans-serif', 'textsize':'15px', 'text':this.props.app_state.loc['900']/* 'Depth-Mint your token ' */+this.state.token_item['id']+this.props.app_state.loc['901']/* ' for a specified target' */})}
                <div style={{height:10}}/>
    ...
    render_everything(){
        return(
            <div>
                ...
                <div style={{height:10}}/>
                <TextInput height={30} placeholder={this.props.app_state.loc['904']/* 'Account ID' */} when_text_input_field_changed={this.when_recipient_input_field_changed.bind(this)} text={this.state.recipient_id} theme={this.props.theme}/>

                {this.load_account_suggestions()}

                {this.render_detail_item('0')}
                {this.render_detail_item('3', {'size':'l', 'details':this.props.app_state.loc['905']/* 'Set the amount to depth-mint' */, 'title':this.props.app_state.loc['890']/* 'Action Amount.' */})}
    ...
    add_transaction(){
        ...
        if(isNaN(recipient) || parseInt(recipient) < 0 || recipient == ''){
            ...
            this.props.notify(this.props.app_state.loc['906']/* 'depth-mint action added!' */, 600)
        }
    }
