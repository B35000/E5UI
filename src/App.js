...
class App extends Component {
  ...
  state = {
    ...
    sync_steps:(90), qr_code_scanning_page:'clear_purchaase', tag_size:23, title_size:65, image_size_limit:5_000_000, ipfs_delay:90, web3_delay:400,
  ...
  get_received_mail_data = async (E52contractInstance, e5, account, web3) => {
    ...
    for(var i=0; i<my_received_mail_events.length; i++){
      ...
      if(mail_activity[convo_id].length != 0){
        received_mail_notifications.push({'type':'mail_message_notification', 'event':my_received_mail_events[i], 'e5':e5, 'timestamp':my_received_mail_events[i].returnValues.p6, 'ipfs':ipfs_obj, 'convo_id':convo_id,'id':cid, 'parent_id':parent_mail_id[convo_id]})
      }else{
        parent_mail_id[convo_id] = cid
      }
